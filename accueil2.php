                  ...
                  <div class="caption">
                      ...
                      <?php if(isset($_SESSION['id_part']) && $_SESSION['role']=="participant"){ ?>
                        <?php
                          ...
                          $id_for=$produit['id_for']; 
                          $id_part=$_SESSION['id_part'];

                          $sql2="SELECT * FROM abonner WHERE id_for=? AND id_part=?";
                          $res2=$bdd->prepare($sql2);
                          $res2->execute(array($id_for,$id_part));
                          $result2 = $res2->fetch();
                          
                        if($result2<=1){
                         ?> 
                          <a class="btn btn-primary" style="width: 100%;"  href="abonner.php?id_for=<?php echo $produit['id_for']; ?>&id_part=<?php echo $_SESSION["id_part"]; ?>">Inscription</a></center>
                     ...
                     ?>
                      <a class="btn btn-danger" style="width: 100%;" <?php 
                         require_once('bd.php');

                        $id_for=$produit['id_for']; 
                        $id_part=$_SESSION['id_part'];

                        $sql3="SELECT * FROM abonner WHERE id_for=? AND id_part=?";
                        $res3=$bdd->prepare($sql3);
                        $res3->execute(array($id_for,$id_part));
                        $result3 = $res3->fetch();
                        if($result3<1){ ?> onclick="return confirm('Auccune inscription enregistrer !!!')"<?php }else{?> href="liste_eval.php?id_for=<?php echo $produit['id_for']; ?>&id_user=<?php echo $_SESSION["id_user"]; ?><?php }?>">Evaluation</a></center>
