...
if ($_SERVER["REQUEST_METHOD"]=="POST") {
	...
	$emailText="";
	if (empty($firstname)){
		$firstnameError="verifier votre prenom";
		$succes=false;
	}
	else{
	...
	if (!isEmail($email)){
		$emailError="votre email n'est pas correct";
		$succes=false;
	}
	else{
	...
	if (!isPhone($phone)){
		$phoneError="votre numéro n'est pas correct";
		$succes=false;
	}
	else{
	...
	if (empty($message)){
		$messageError="votre message est vide";
		$succes=false;
	}
	else{
	...
	if ($succes) {
		$headers="From: {$firstname} {$name} <{$email}>\r\Reply-To:{$email}";
		mail($emailTo,"merci de m'avoir contacter",$emailText, $headers);
		$firstname=$name=$email=$phone=$message="";
